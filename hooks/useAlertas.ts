...
import { AlertaOutput, apiGetAlertasByUserId } from '../services/api';
...
export function useAlertas() {
  const query = useQuery<AlertaOutput[]>({
    queryKey: ['alertas', 'usuario'],
    queryFn: async () => {
      const { token, userId } = await getSessionData();
      return apiGetAlertasByUserId(userId, token);
    },
    staleTime: 1000 * 30,
  });
